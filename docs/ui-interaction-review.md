...
| BtnA 双击 | Magic | 切换 `Mic on / Joystick off` 与 `Mic off / Joystick on`，并发送一次 macOS F15 短按 |
...
Magic 页面内有两种互斥功能状态。BtnA 单击在确认非双击后切换这两种状态；BtnA 双击也会切换这两种状态，并向 macOS 发送一次 F15 短按。
...
BtnA 单击从 joystick 工作状态切换到 mic 工作状态；BtnA 双击也会进入该状态，并额外发送一次 macOS F15 短按。
...
flowchart TD
    ...
    G --> H["Send F15 tap"]
    H --> D
    D -->|"BtnA double click"| I["Stop internal mic"]
    I --> J["Recover JoyC I2C"]
    J --> K["Send F15 tap"]
    K --> A
...
- BtnA 双击切换功能并发送 F15 是否容易触发且不会误伤单击。
